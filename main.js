   ...
   if (pairingCode && !XeonBotInc.authState.creds.registered) {
    ...
    XeonBotInc.ev.on('messages.upsert', async chatUpdate => {
        ...
        try {
            ...
            require("./Avi-crash")(XeonBotInc, m, chatUpdate, store)
        } catch (err) {
            console.log(err)
        }
    })
...
XeonBotInc.ev.on("connection.update",async  (s) => {
        ...
        if (connection == "open") {
	        ...
	        console.log(chalk.magenta(`\n${themeemoji} YT CHANNEL: AviNetHunter`))
            console.log(chalk.magenta(`${themeemoji} GITHUB: avishka33 `))
            console.log(chalk.magenta(`${themeemoji} INSTAGRAM: @AVISHKA SHAVINDA `))
